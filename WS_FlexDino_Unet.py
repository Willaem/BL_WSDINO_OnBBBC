...
from datasets import BBBC021_3ChannelSet
...
def get_args_parser():
    ...
    parser.add_argument('--arch', default='vit_small', type=str,
        choices=['vit_tiny', 'vit_small', 'vit_base', 'xcit', 'deit_tiny', 'deit_small', 'unet'] \
                + torchvision_archs,
...
def makeWeights(args):
    ...
    return 1/torch.tensor(weight_list_flat)
...
def train_dino(args):
    ...
    dataset = BBBC021_3ChannelSet(path_to_ref_sheet, local_crops_number= args.local_crops_number)
    ...
    # otherwise, we check if the architecture is in torchvision models
        ...
        student = torch.hub.load('milesial/Pytorch-UNet', 'unet_carvana', pretrained=False, scale=0.5)
        teacher = torch.hub.load('milesial/Pytorch-UNet', 'unet_carvana', pretrained=False, scale=0.5)
    else:
...
#        data_loader.sampler.set_epoch(epoch)
        ...
        utils.save_on_master(save_dict, os.path.join(args.output_dir, f'{args.arch}_unifiedChannels_{args.weak_label_header}_DINO_checkpoint.pth'))
        if args.saveckp_freq and epoch % args.saveckp_freq == 0:
            utils.save_on_master(save_dict, os.path.join(args.output_dir, f'{args.arch}_unifiedChannels_{args.weak_label_header}_DINO_checkpoint{epoch:04}.pth'))
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     'epoch': epoch}
        if utils.is_main_process():
            with (Path(args.output_dir) / f"{args.arch}_unifiedChannels_{args.weak_label_header}_DINO_log.txt").open("a") as f:
